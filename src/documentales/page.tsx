...
export default function Page() {
  const docs = TITLES.filter(t => t.genres.some(g => /documental/i.test(g))).map(titleToItem);
  return (
    <main>
      <Navbar />
      <div className="mx-auto max-w-6xl px-4 py-8">
        <h1 className="text-2xl font-bold mb-6">Documentales</h1>        
        <Row title="Destacados" items={docs} />
      </div>
    </main>
  );
}
